...
const GruposPage = () => {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Cargando grupos...</div>;
    ...
    <div className="p-6 md:p-10 min-h-screen flex flex-col bg-[#F9FAFB]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {grupos.length === 0 ? (
          ...
          grupos.map((grupo) => (
            <div key={grupo.id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{grupo.estudiantes?.length || 0} estudiantes</p>
                </div>
              </div>
              ...
              {isDocente && (
                <div className="bg-slate-50 rounded-xl p-4 flex items-center justify-between border border-slate-100">
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Código de acceso</p>
                    <p className="font-mono font-bold text-slate-900 tracking-widest">{grupo.codigo_acceso}</p>
                  </div>
                  <button 
                    onClick={() => handleCopyCode(grupo.codigo_acceso)}
                    className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-indigo-100 shadow-sm"
                    title="Copiar código"
                  >
              ...
              {!isDocente && grupo.docente && (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 text-sm text-slate-600">
                  <span className="font-medium text-slate-900">Docente:</span> {grupo.docente.nombre}
                </div>
              )}
            </div>
          ))
                ...
                {isDocente ? (
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      className="w-full font-mono text-center tracking-widest px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all uppercase"
                    />
                  </div>
                )}
